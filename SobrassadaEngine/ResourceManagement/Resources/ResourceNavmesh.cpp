...
)
{
    ...
    if (userConfig.GetSettings().filterLowHangingObstacles)
        rcFilterLowHangingWalkableObstacles(context, config.walkableClimb, *heightfield);
    if (userConfig.GetSettings().filterLedgeSpans)
        rcFilterLedgeSpans(context, config.walkableHeight, config.walkableClimb, *heightfield);
    if (userConfig.GetSettings().filterWalkableLowHeightSpans)
...
void ResourceNavMesh::CreateDetourData(
    const rcPolyMesh* polymesh, const rcPolyMeshDetail* polymeshDetail, const rcConfig& config
)
{
    if (!polymesh) return;
