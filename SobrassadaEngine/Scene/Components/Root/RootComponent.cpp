...
void RootComponent::RenderComponentEditor()
{
    ...
    if (selectedUID != uid)
    {
        ...
        if (ImGui::Button("Remove Component"))
        {
            ...
            {
                Component* selectedParentComponent =
                    App->GetSceneModule()->GetComponentByUID(selectedComponent->GetUIDParent());
...
void RootComponent::RenderEditorComponentTree(const UID selectedComponentUID)
{
    ...
    if (ImGui::IsItemClicked() && !ImGui::IsItemToggledOpen()) SetSelectedComponent(uid);
...
bool RootComponent::CreateComponent(const ComponentType componentType)
{
    ...
    if (selectedComponent != nullptr)
    {
        Component* createdComponent = ComponentUtils::CreateEmptyComponent(
            componentType, LCG().IntFast(), selectedUID, uid, selectedComponent->GetGlobalTransform()
        );
